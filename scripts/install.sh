...
# Ferronote Installer Script
# Supported Platforms: macOS, Linux (x86_64, arm64)
# Usage: curl -fsSL https://raw.githubusercontent.com/tareqmy/ferronote/master/scripts/install.sh | sh
...
resolve_version() {
    if [ "${VERSION:-}" = "" ]; then
        ...
        if [ -f ".version" ] && [ -d ".git" ] && grep -q "ferronote" Cargo.toml 2>/dev/null; then
        ...
        if [ -z "${LATEST_RELEASE:-}" ]; then
            error "Could not auto-detect latest version. Please specify the version manually, for example: VERSION=v1.1.3 ./install.sh"
        fi
...
download_and_extract() {
    DOWNLOAD_URL="${GITHUB_RELEASES_URL}/download/${VERSION}/ferronote-${VERSION}-${TARGET}.tar.gz"
    
    # Create temporary directory
    TMP_DIR=$(mktemp -d -t ferronote-install.XXXXXX)
    trap 'rm -rf "${TMP_DIR}"' EXIT INT TERM

    ARCHIVE_PATH="${TMP_DIR}/ferronote.tar.gz"
    ...
    if [ -n "${GITHUB_TOKEN:-}" ]; then
        ...
        # Public download
        info "Downloading Ferronote from: ${DOWNLOAD_URL}"
        if command -v curl >/dev/null 2>&1; then
    ...
    BINARY_PATH=$(find "${TMP_DIR}" -type f -name "ferronote" -perm -111 -print -quit || find "${TMP_DIR}" -type f -name "ferronote" -print -quit || true)
    if [ -z "${BINARY_PATH}" ] || [ ! -f "${BINARY_PATH}" ]; then
        error "Binary 'ferronote' not found in the downloaded archive."
    fi

    # Install binary
    info "Installing ferronote to ${INSTALL_DIR}..."
...
# Quick test run
if command -v "${INSTALL_DIR}/ferronote" >/dev/null 2>&1; then
    VERSION_OUT=$("${INSTALL_DIR}/ferronote" --version 2>&1 || echo "installed successfully")
