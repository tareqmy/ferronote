# Ferronote Installer for Windows
# Supported Platforms: Windows (x86_64)
# Usage: irm https://raw.githubusercontent.com/tareqmy/ferronote/master/scripts/install.ps1 | iex
...
if ($arch -eq "AMD64") {
    ...
    Write-ErrorMsg "Unsupported Windows architecture: $arch. Ferronote currently only supports x86_64 Windows."
}
...
$download_url = "$github_releases_url/download/$version/ferronote-$version-$target.zip"
$tmp_dir = Join-Path $env:TEMP "ferronote-install-$([Guid]::NewGuid())"
New-Item -ItemType Directory -Path $tmp_dir -Force | Out-Null

$zip_path = Join-Path $tmp_dir "ferronote.zip"

Write-Info "Downloading Ferronote from $download_url..."
try {
    ...
    if ($env:GITHUB_TOKEN) {
        ...
        $asset_name = "ferronote-$version-$target.zip"
        
        $asset = $release_json.assets | Where-Object { $_.name -eq $asset_name }
        if (-not $asset) {
            $alt_name = "ferronote-$target.zip"
...
# Find ferronote.exe and move it
$exe_path = Join-Path $tmp_dir "ferronote.exe"
if (-not (Test-Path $exe_path)) {
    # Try looking inside subfolders in case of double packaging
    $exe_path = Get-ChildItem -Path $tmp_dir -Filter "ferronote.exe" -Recurse | Select-Object -First 1
}

if (-not $exe_path) {
    Write-ErrorMsg "Could not find ferronote.exe in the extracted archive."
}

$dest_path = Join-Path $install_dir "ferronote.exe"
